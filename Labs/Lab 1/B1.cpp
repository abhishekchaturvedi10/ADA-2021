    ...
    loopf(i, 0, n - 1) {
        loopf(j, i, n - 1) {
            ...
            loopf(k, i, n - 1) {
                if (k == i || j == k)
                    continue;
                ld cur = sqrt((a[i].ff - a[j].ff) * (a[i].ff - a[j].ff) + (a[i].ss - a[j].ss) * (a[i].ss - a[j].ss));
                cur += sqrt((a[i].ff - a[k].ff) * (a[i].ff - a[k].ff) + (a[i].ss - a[k].ss) * (a[i].ss - a[k].ss));
                cur += sqrt((a[j].ff - a[k].ff) * (a[j].ff - a[k].ff) + (a[j].ss - a[k].ss) * (a[j].ss - a[k].ss));
                ans = min(ans, cur);
            }
        }
    }
